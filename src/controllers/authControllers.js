...
async function login(req, res) {
  ...
  if (user.two_factor_enabled) {
    ...
    const tempToken = jwt.sign({ userId: user.id, step: '2fa' }, process.env.JWT_SECRET, { expiresIn: '10m' });
...
async function enable2FA(req, res) {
  ...
  res.json({
    message: 'Check your email for QR code to scan. If not visible, use this secret manually in your app.',
